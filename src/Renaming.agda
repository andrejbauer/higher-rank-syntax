...
module Renaming (Sort : Set) where
  ...
  [ ρ ]ʳ (x ` ts) = ρ x ` λ i → [ ⇑ʳ ρ ]ʳ ts i

  -- the action respects equality of renamings and equality of terms

  []ʳ-resp-≈ : ∀ {Γ Δ A} (ρ : Γ →ʳ Δ) {t u : Expr Γ A} → t ≈ u → [ ρ ]ʳ t ≈ [ ρ ]ʳ u
  []ʳ-resp-≈ ρ (≈-≡ ξ) = ≈-≡ (cong ([ ρ ]ʳ_) ξ)
  []ʳ-resp-≈ ρ (≈-` ξ) = ≈-` (λ i → []ʳ-resp-≈ (⇑ʳ ρ) (ξ i))

  []ʳ-resp-≡ʳ : ∀ {Γ Δ A} {ρ τ : Γ →ʳ Δ} (t : Expr Γ A) → ρ ≡ʳ τ → [ ρ ]ʳ t ≈ [ τ ]ʳ t
  []ʳ-resp-≡ʳ (x ` ts) ξ = ≈-trans (≈-≡ (cong (_` _) (ξ x))) (≈-` (λ i → []ʳ-resp-≡ʳ (ts i) (⇑ʳ-resp-≡ʳ ξ)))

  []ʳ-resp-≡ʳ-≈ : ∀ {Γ Δ A} {ρ τ : Γ →ʳ Δ} {t u : Expr Γ A} → ρ ≡ʳ τ → t ≈ u → [ ρ ]ʳ t ≈ [ τ ]ʳ u
  []ʳ-resp-≡ʳ-≈ ζ ξ = ≈-trans ([]ʳ-resp-≡ʳ _ ζ) ([]ʳ-resp-≈ _ ξ)

  -- the action is functorial

  [𝟙ʳ] : ∀ {Γ A} {t : Expr Γ A} → [ 𝟙ʳ ]ʳ t ≈ t
  [𝟙ʳ] {t = x ` ts} = ≈-` (λ i → ≈-trans ([]ʳ-resp-≡ʳ _ ⇑ʳ-resp-𝟙ʳ) [𝟙ʳ])

  [∘ʳ] : ∀ {Γ Δ Θ A} {ρ : Γ →ʳ Δ} {τ : Δ →ʳ Θ} (t : Expr Γ A) → [ τ ∘ʳ ρ ]ʳ t ≈ [ τ ]ʳ [ ρ ]ʳ t
  [∘ʳ] (x ` ts) = ≈-` (λ i → ≈-trans ([]ʳ-resp-≡ʳ _ ⇑ʳ-resp-∘ʳ) ([∘ʳ] (ts i)))
  ...
  module _ where
    ...
    Renamings =
     record
       ...
       ; identityʳ = λ x → refl
       ; identity² = λ x → refl
       ; equiv = record { refl = λ x → refl ; sym = ≡ʳ-sym ; trans = ≡ʳ-trans }
