...
module Renaming (Sort : Set) where
  ...
  _→ʳ_ Γ Δ = ∀ {Ξ : Shape} {A} (x : [ Ξ , A ]∈ Γ) → [ Ξ , A ]∈ Δ

  -- equality of renamings
  infix 5 _≡ʳ_

  _≡ʳ_ : ∀ {Γ : Shape} {Δ : Shape} → (ρ : Γ →ʳ Δ) → (τ : Γ →ʳ Δ) → Set
  _≡ʳ_ {Γ = Γ} ρ τ = ∀ {Ξ : Shape} {A} (x : [ Ξ , A ]∈ Γ) → ρ x ≡ τ x

  -- equality is an equivalence relation

  ≡ʳ-refl : ∀ {Γ : Shape} {Δ : Shape} → {ρ : Γ →ʳ Δ} → ρ ≡ʳ ρ
  ≡ʳ-refl x = refl

  ≡ʳ-sym : ∀ {Γ : Shape} {Δ : Shape} → {ρ τ : Γ →ʳ Δ} → ρ ≡ʳ τ → τ ≡ʳ ρ
  ≡ʳ-sym ξ x = sym (ξ x)

  ≡ʳ-trans : ∀ {Γ : Shape} {Δ : Shape} → {ρ τ χ : Γ →ʳ Δ} → ρ ≡ʳ τ → τ ≡ʳ χ → ρ ≡ʳ χ
  ...
  _∘ʳ_ : ∀ {Γ : Shape} {Δ : Shape} {Θ : Shape} → (Δ →ʳ Θ) → (Γ →ʳ Δ) → (Γ →ʳ Θ)
  (ρ ∘ʳ τ) x =  ρ (τ x)
