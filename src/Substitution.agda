...
module Substitution (Class : Set) where
  ...
  module _ where
    ...
    app-cong-≡' : ∀ {Γ} {n} {u v : ∀ {Δ A} (g : Γ →ˢ Δ) (e : Arg Δ Γ A) → size e ≡ n → Expr Δ A} →
                  (λ {Δ = Δ} {A = A} → u {Δ = Δ} {A = A}) ≡ v → u ≡' v
    app-cong-≡' refl g e ξ = refl

    postulate funext : ∀ {ℓa ℓb} {A : Set ℓa} {B : A → Set ℓb} {f g : ∀ x → B x} →
                         (∀ x → f x ≡ g x) → f ≡ g

    postulate funext' : ∀ {ℓa ℓb} {A : Set ℓa} {B : A → Set ℓb} {f g : ∀ {x} → B x} →
                          (∀ x → f {x = x} ≡ g {x = x}) → (λ {x} → f {x = x}) ≡ g

    funext₂ : ∀ {ℓa ℓb ℓc} {A : Set ℓa} {B : A → Set ℓb} {C : ∀ x (y : B x) → Set ℓc} {f g : ∀ x y → C x y} →
                         (∀ x y → f x y ≡ g x y) → f ≡ g
    funext₂ ξ = funext (λ x → funext (λ y → ξ x y))

    funext₂' : ∀ {ℓa ℓb ℓc} {A : Set ℓa} {B : A → Set ℓb} {C : ∀ x (y : B x) → Set ℓc} {f g : ∀ {x} {y} → C x y} →
                         (∀ x y → f {x = x} {y = y} ≡ g {x = x} {y = y}) → (λ {x} {y} → f) ≡ g
    funext₂' ξ = funext' (λ x → funext' (λ y → ξ x y))

    open FixPoint wf-≺,< P b
           (λ Γ,n {r₁} {r₂} ζ →
              funext₂' (λ Δ A →
              funext (λ f →
              funext (λ { (var-left x ` ts) →
                            funext (λ ξ → cong (_`_ x)
                            (funext₂' (λ Ξ B →
                             funext (λ y →
                               {!app-cong-≡'!}
                            ))))
    ...
    -- b-ext (Γ , m) ζ g (var-left x ` _) _ = cong (_`_ x) ( (argext λ y → ζ _ _ _ refl) )
    -- b-ext (Γ , m) {r₁} {r₂} ζ g (var-right x ` ts) ξ =
    --   trans
    --     (cong-app (cong-app (cong (r₁ _ _) (argext (λ y → ζ _ _ _ refl))) _) _)
    --     (ζ _ _ _ refl)

    -- open import FixPointRel wf-≺,< lzero P b _≡'_ b-ext

    unfold-inst-b : ∀ {Γ Δ A} {f : Γ →ˢ Δ} {e : Arg Δ Γ A} → inst f e ≡ b (Γ , size e) (λ Ω,n ξ → wfRec P b Ω,n) f e refl
    unfold-inst-b {f = f} {e = e} = {!!}
