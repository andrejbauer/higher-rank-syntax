...
1. **[2026-09-21] Telescope equality follows prefixes and needs ambient transport**
   Do instead: recurse on the first telescope or its `Wf_t` derivation, read the other side through `Eq_t.cons_inv`, and use `Eq_t.Both.symm` with `Wf_sub.ofBoth` to move lifted substitutions between equal extensions.
2. **[2026-08-31] Semantic categories use quotients, not Setoid enrichment**
   Do instead: quotient substitutions, telescopes, and related data by their specified equivalences throughout, and formulate `Ctx`, presheaves, limits, and CwRs as ordinary 1-categorical objects.
3. **[2026-08-08] Distinguish ordinary and protected-prefix lifting**
...
4. **[2026-08-03] Prove telescope-tensor coherence through base extension**
...
5. **[2026-08-03] Use HEq at dependent telescope-tensor projections**
...
6. **[2026-08-03] Telescope reindexing uses a specialized pushforward**
...
7. **[2026-06-23] Carrier product slots need cover**
   Do instead: use `Carrier.cover` to split arbitrary `Γ ⋈ Δ` slots; `classify_inl`/`classify_inr` only handle injected slots.
8. **[2026-08-03] Compare dependent decoration paths through packed sites**
...
9. **[2026-08-03] Eliminate dependent product-slot data through a sigma**
...
10. **[2026-06-24] Reassociate nested product slots with Carrier coherences**
...
## User Directives
1. **[2026-09-08] Rebuild SOGAT comparisons from the semantic definitions**
   Do instead: treat `rank-and-sogats.md` and `initiality.md` as provisional; start from `semantics.md`, check the full `Ctx / Ξ` quantifier, and allow reconstructed sorts to be full semantic function spaces rather than only named carriers.
2. **[2026-09-02] Use no LaTeX markup in Markdown documents**
...
3. **[2026-08-10] Keep foundational syntax discussions exploratory and explicit about uncertainty**
...
4. **[2026-08-03] Implement staged roadmaps one pass per turn**
...
5. **[2026-06-23] Keep Lean answers practical**
   Do instead: include a minimal compiling example and explain the key imported names.
6. **[2026-09-21] Remove auxiliary proof files when finished**
   Do instead: keep temporary checks in `/tmp` and delete the files created for the task after validation.
