...
An argument family can be viewed as a singleton substitution. In Lean, such a
family has type `Expr.Args Ω β`. If `A` is an argument family for a head of
arity `β`, write
...
This is Lean's `Subst.fromArgs β Ω A`; conversely, `Subst.toArgs` reads a
singleton-domain substitution as its argument family. The β-like step is then
not a new primitive operation: it is ordinary substitution action by `A♯`. Thus
...
This is the postcomposition of $κ$ by $σ$. For a slot $x : S ∋ γ$, its filler is
...
First unfold the left-hand side. The head $xₛ$ is passive for $σ$, because
$σ$ only replaces variables from $D$. Therefore $σ$ preserves the head and
acts on the arguments:
...
Now apply $ρ_U$. This substitution acts on $xₛ$, so it fires. Firing means:
take the filler $ρ(xₛ)$ and substitute into its freshly opened $β$-binder using
the transformed argument family. Define
...
and $Āⱼ$ is in the domain. Therefore

$$
Lⱼ ∈ 𝓔(P ⋈ D' ⋈ T ⋈ S' ⋈ Uⱼ).
$$

The family $j ↦ Lⱼ$ gives a singleton-domain substitution
...
The input $ρ(xₛ)$ has exactly the required domain type. Hence
...
The filler being fired is $κ(xₛ)$, and we already recorded that
...
Its input $κ(xₛ)$ has exactly this domain type, so
...
Applying $σ_{T ⋈ S' ⋈ U}$ is now well-typed, since
...
In Lean, the singleton substitution `(κ_U · a)♯` is
`Subst.actPointwise kappa upsilon args`.
...
where `Subst.actPointwise kappa upsilon args` denotes `(κ_U · a)♯`.
...
Its filler at `here(j)` is, by definition of postcomposition,

$$
\begin{aligned}
&
(σ ∘^{T ⋈ S' ⋈ U} (κ_U · a)♯)(here(j))
\\
&\qquad =
σ_{T ⋈ S' ⋈ Uⱼ}
(
...
This is exactly the expression that we call `Rⱼ` below. Thus the postcomposed
singleton substitution is `R♯`. Consequently its action at empty passive depth
is
...
has precisely this domain type, because $κ(xₛ)$ lives in
$𝓔(P ⋈ D ⋈ T ⋈ S' ◁ β)$. Therefore the left-hand side of the recursive
diamond is exactly
...
So, after naming this postcomposed singleton substitution `R♯`, the recursive
...
This is well-typed because $κ_{Uⱼ}(a(j))$ lives in
$𝓔(P ⋈ D ⋈ T ⋈ S' ⋈ Uⱼ)$, so
...
$a(j)$ say precisely that. Fix one binder
...
The ordinary argument-recursive call of `diamondAt` is made with the same
substitutions $σ$ and $κ$, the same telescopes
...
The left-hand side is exactly $Lⱼ$, because
...
The right-hand side is exactly $Rⱼ$, by the definition of $Rⱼ$. Hence
`hargs j` is precisely the equality
...
The equality named `hSubst` in Lean is the extensional equality between the two
singleton-domain substitutions generated by the two argument families:

$$
L♯ = R♯.
$$

Both substitutions have domain $⟨β⟩$ and codomain
...
which unfolds to $Lⱼ = Rⱼ$, i.e. exactly `hargs j`. This is why the Lean proof
of `hSubst` has one real case, `here(j)`, and that case is closed by
`hargs j`.
