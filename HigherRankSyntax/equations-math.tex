...
The substitution-level composition operation is now the general
postcomposition-at-depth operation
\[
  \Code{Subst.comp}\;\theta\;\Omega\;\sigma,
\]
where \(\Omega\) is a passive telescope.  Pointwise,
\[
  (\Code{Subst.comp}\;\theta\;\Omega\;\sigma)(x)
  =
  \Act{\theta}{\Omega\comp\langle\alpha\rangle}{\sigma(x)}
  \qquad(x : \dom \ni \alpha).
\]
The ordinary Kleisli-style composition used in \Code{Subst.act\_comp} is the
empty-depth specialization
\[
  \Code{Subst.kcomp}\;\sigma\;\theta
  :=
  \Code{Subst.comp}\;\theta\;\Code{Shape.nil}\;\sigma.
\]
...
\[
  \Act{\Code{Subst.kcomp}\;\sigma\;\theta}{\tau}{e}
  =
...
\Code{Subst.comp} & Postcomposition at passive depth; ordinary Kleisli
composition is the empty-depth case. & Postcomposition of syntactic
transformations. \\

\Code{Subst.kcomp} & Kleisli composition, i.e. the empty-depth specialization
of \Code{Subst.comp}. & Composition of syntactic transformations. \\
