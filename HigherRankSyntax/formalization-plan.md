...
## A7.9 — 8(11), equivalence — **done**

*Produces.* `Typing/Equivalence.lean`:

```lean
theorem Eq_t.symm  (hΞ : Ambient.Wf Ξ) : Eq_t Ξ Θ Θ' → Eq_t Ξ Θ' Θ
theorem Eq_t.trans (hΞ : Ambient.Wf Ξ) :
    Wf_t Ξ Θ → Eq_t Ξ Θ Θ' → Eq_t Ξ Θ' Θ'' → Eq_t Ξ Θ Θ''
```

`Eq_t.symm` is `Eq_t.Both.symm` read through `Eq_t.toBoth` and `Eq_t.Both.toEq_t`.
`Eq_t.trans` recurses on the `Wf_t` derivation and projects both equalities by
`Eq_t.cons_inv`; each step moves the second comparison from the ambient built from
the middle telescope to the one built from the first, by `Eq_bd.ofEq` resp.
`Eq_t.ofEq` along `Eq_t.concatenate (Wf_t.refl hΞ) (Eq_t.symm hΞ …)`.  Reflexivity
is `Wf_t.refl`.

Supporting: `Eq_t.ofBoth`, `Eq_t.ofEq` and `boundaryOf_ofEq` in
`Typing/Invariance.lean`; and, in `Typing/Equivalence.lean`, the preservation
lemmas `Wf_bd.ofEq_bd` (a well-formed declaration stays well formed at an equal
boundary), `Wf_bd.ofEq_t` (…over equal bound entries) and `Wf_t.ofEq_t` (a
well-formed telescope stays well formed at an equal one) — `Eq_t.trans` needs
well-formedness of the middle telescope, which it cannot invert alongside the
first.

*Also done.* `Wf_sub.id` (8(5) in the 6.8 form — the identity substitution between
ambients is well formed, its equational obligation discharged by clause 6.5(2) at
the slot itself), `Wf_s.toSub` and `Eq_s.toSub` (a filling of `Θ` over `Ξ` is the
substitution `Ξ ⋈ Θ ⟶ Ξ` given by `Subst.copair (Subst.id Δ) σ`), and hence 8(9)'s
second half in the filling form:

```lean
theorem Eq_t.agree_fill (hΞ : Ambient.Wf Ξ) (hΘ : Wf_t Ξ Θ) (hσ : Wf_s Ξ Θ σ)
...
Three lines, because `dTel.actBase (Subst.copair (Subst.id Δ) σ) X` is
definitionally `dTel.instantiate σ X`: the 6.8 statement and the filling statement
are the same proposition once the substitution is converted.  `Wf_s.lift` and
`Eq_s.lift` are not needed.

*Also done.* `Eq_s.symm` and `Eq_s.trans`, closing 8(11):

```lean
theorem Eq_s.symm  (hΞ : Ambient.Wf Ξ) :
    Eq_s Ξ Θ σ θ → Wf_t Ξ Θ → Wf_s Ξ Θ σ → Wf_s Ξ Θ θ → Eq_s Ξ Θ θ σ
theorem Eq_s.trans (hΞ : Ambient.Wf Ξ) :
    Eq_s Ξ Θ σ θ → Eq_s Ξ Θ θ κ → Wf_t Ξ Θ → Wf_s Ξ Θ σ → Wf_s Ξ Θ θ → Eq_s Ξ Θ σ κ
```

Both recurse on the first `Eq_s` derivation — only its `cons` field sits at the
telescope the goal names — and take everything else by projection: `Wf_s.head`,
`Wf_s.tail`, `Eq_s.slot`, `Eq_s.tail`, and `Wf_t.cons_inv`.  The head comparisons
are over `Ξ ⋈ bind`, which mentions neither substitution, so they are `Eq_e.symm`
and `Eq_e.trans`.  The tails cross from `σ ⋆ rest` to `θ ⋆ rest` by `Eq_t.agree_fill`
and `Eq_s.ofBoth`, with `Wf_s.ofEq` moving the second filling across the same
equality.

`Wf_t.cons_inv` is what made this possible.  `Wf_t` must stay inductive — it is a
premise of `Eq_e.congr` — and cannot be inverted at a `cons` telescope, so its
components are extracted through a private `Wf_t.parts`, a `def` by recursion on
the telescope whose value at a `cons` is the triple; `Wf_t.toParts` is then a
one-line recursion on the derivation.  The same device would serve any other
inductive judgement that needs decomposing at a known telescope.

Also added: `Wf_t.instantiate` (8(9) with no suffix).
...
is setoid-first; this development is not — see the standing decision.  Equational
slots are deliberately not compared: nothing constrains them (6.3) and nothing
depends on them.
