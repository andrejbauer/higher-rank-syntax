...
theorem act_ap_depth {A : Type} {C : Carrier A} {Γ Δ Ξ : C.Arity}
  ...
  · congr 1
    convert (Proper.inr_inl (Λ ⋈ Φ) Ρ (Γ ⋈ Δ) (Proper.inr Λ z)).symm using 1
    congr 1
    symm
    apply Proper.inr_inr
  · congr 1
    · convert (Proper.inr_inl (Λ ⋈ Φ) Ρ (Γ ⋈ Ξ) (Proper.inr Λ z)).symm using 1
      congr 1
      symm
      apply Proper.inr_inr
    · funext j
...
theorem act_interchange.subst {C : Carrier} {Γ Λ Θ Ψ Ω Φ Χ : Shape C}
  ...
  | .ap (α := β) x args =>
    ...
    case left =>
      rw [act_left]
      convert act_left θ (Φ ⋈ Χ) (Proper.inl Γ z) _ using 2
      rw [act_left]
      congr 1
      funext i
      symm
