...
`xs` together with the binder it opens.  This is the pure list-level theory,
...
`idx` and the binder `tag` it picks out.  `ext` is the resulting determinacy
principle.
-/
...
/-- The binder a slot picks out, together with the list element it sits at. -/
def tag {C : Carrier} : {xs : List C.Arity} → {α : C.Arity} →
    ListSlotAt xs α → Σ β : C.Arity, C.Binder β
  | _, _, .here (β := β) i => ⟨β, i⟩
  | _, _, .there s         => s.tag

/-- The arity of a slot is the arity of the binder it picks out. -/
...
binder are equal, even when their arity indices are stated separately. -/
...
/-- A slot is determined by its position and the binder it picks out. -/
