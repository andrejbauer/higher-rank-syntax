...
private abbrev Subst.actPointwise
  ...
  Subst.fromArgs β ((pre ⋈ dst) ⋈ Tele.ofList υ)
...
private abbrev sequential
  ...
  κ.act ((Tele.ofList υ) ⋈ Tele.ofList χ)
    (Subst.act (Γ := pre) (Ξ := (τ ⋈ src) ⋈ Tele.ofList υ)
      (Subst.fromArgs β (pre ⋈ ((τ ⋈ src) ⋈ Tele.ofList υ)) η)
      (Tele.ofList χ) e)
...
private abbrev fused
    ...
    (η : (j : C.Binder β) →
      Expr (((pre ⋈ τ) ⋈ src) ⋈ Tele.ofList υ ∷ j.arity))
    (e : Expr ((pre ∷ β) ⋈ Tele.ofList χ)) :=
  Subst.act (Γ := pre) (Ξ := (τ ⋈ dst) ⋈ Tele.ofList υ)
    (Subst.fromArgs β (pre ⋈ ((τ ⋈ dst) ⋈ Tele.ofList υ))
...
private theorem DiamondSite.actBySubst {C : Carrier}
    ...
    | .tau xτ =>
        .ap
          ...
          (fun j => σ.act (((τ ⋈ src) ⋈ Tele.ofList υ) ∷ j.arity) (args j))
    | .dom z =>
        Subst.act (Γ := pre ⋈ cod) (Ξ := (τ ⋈ src) ⋈ Tele.ofList υ)
          (Subst.fromArgs β ((pre ⋈ cod) ⋈ ((τ ⋈ src) ⋈ Tele.ofList υ))
            (fun j => σ.act (((τ ⋈ src) ⋈ Tele.ofList υ) ∷ j.arity) (args j)))
          Shape.nil
          (σ z)
    | .pre z =>
  ...
  | .ap (α := γ) head args =>
      ...
      | beta j =>
          ...
          let θSubst : Subst C ⌊j.arity⌋ (pre ⋈ τ ⋈ src ⋈ Tele.ofList υ ⋈ Tele.ofList χ) :=
            Subst.fromArgs j.arity
              (pre ⋈ τ ⋈ src ⋈ Tele.ofList υ ⋈ Tele.ofList χ)
...
theorem Subst.act_comp
  ...
  | .ap (α := β) head args =>
    ...
    | middle y =>
      ...
      let κβ : Subst C ⌊β⌋ (pre ⋈ mid ⋈ τ) :=
        Subst.fromArgs β (pre ⋈ mid ⋈ τ)
