...
β branches. -/
private abbrev Subst.fromArgs
    {C : Carrier} {pre : Shape C} (α : C.Arity) (cod : Shape C)
    (f : (i : C.Binder α) → Expr ((pre ⋈ cod) ∷ i.arity)) :
    Subst C ⌊α⌋ (pre ⋈ cod) :=
  (fun | .here i => f i)

/- Apply a substitution pointwise to an argument family, then package the
resulting family as the singleton substitution generated by those acted
arguments.  In the notes this is `(κ_U · args)♯`. -/
private abbrev Subst.actPointwise
    {C : Carrier} {pre src dst : Shape C}
    [Proper src] [Proper dst]
    (κ : Subst C src (pre ⋈ dst))
    {β : C.Arity} (υ : List C.Arity)
    (args :
      (j : C.Binder β) →
        Expr (((pre ⋈ src) ⋈ Tele.ofList υ) ∷ j.arity)) :
    Subst C ⌊β⌋ ((pre ⋈ dst) ⋈ Tele.ofList υ) :=
  Subst.fromArgs β (Tele.ofList υ)
    (pre := pre ⋈ dst)
...
private abbrev sequential
  ...
  κ.act ((Tele.ofList υ) ⋈ Tele.ofList χ)
    (Subst.act (Subst.fromArgs β ((τ ⋈ src) ⋈ Tele.ofList υ) η) (Tele.ofList χ) e)
...
private abbrev fused
    ...
    (η : (j : C.Binder β) →
      Expr (((pre ⋈ τ) ⋈ src) ⋈ Tele.ofList υ ∷ j.arity))
    (e : Expr ((pre ∷ β) ⋈ Tele.ofList χ)) :=
  Subst.act (Subst.fromArgs β ((τ ⋈ dst) ⋈ Tele.ofList υ)
...
private theorem DiamondSite.actBySubst {C : Carrier}
    ...
    | .tau xτ =>
        .ap
          ...
          (fun j => σ.act (((τ ⋈ src) ⋈ Tele.ofList υ) ∷ j.arity) (args j))
    | .dom z =>
        ⟦ Subst.fromArgs β ((τ ⋈ src) ⋈ Tele.ofList υ)
  ...
  | tau xτ =>
      ...
      rfl
  | dom z =>
      unfold actAt DiamondSite.embed Subst.fromArgs
...
/- Stable under-list heads in `diamondAt` are purely argumentwise: both
routes preserve the same under-head, and the recursive hypotheses prove the
argument families equal. -/
private theorem DiamondSite.underBranch {C : Carrier}
    {pre dom cod τ src dst : Shape C}
    ...
    (κσ : Subst C src (((pre ⋈ cod) ⋈ τ) ⋈ dst))
    {β : C.Arity}
    (xυ : Tele.ofList υ ∋ β)
    (args : (j : C.Binder β) →
      Expr (((((pre ⋈ dom) ⋈ τ) ⋈ src) ⋈ Tele.ofList υ) ∷ j.arity))
    (hargs : ∀ (j : C.Binder β),
      Subst.act κσ (Tele.ofList (j.arity :: υ))
        (σ.act ((τ ⋈ src) ⋈ Tele.ofList (j.arity :: υ)) (args j))
      =
      σ.act ((τ ⋈ dst) ⋈ Tele.ofList (j.arity :: υ))
        (κ.act (Tele.ofList (j.arity :: υ)) (args j))) :
    Subst.act κσ (Tele.ofList υ)
      (actAt (pre := pre) σ (Proper.underList hτSrc υ)
        (.ap (DiamondSite.embed hτSrc (.under xυ)) args))
    =
    σ.act ((τ ⋈ dst) ⋈ Tele.ofList υ)
      (κ.act (Tele.ofList υ)
        (.ap (DiamondSite.embed hτSrc (.under xυ)) args)) := by
...
/- Stable ambient-depth heads in `diamondAt` are also argumentwise.  The head
comes from `τ`, so neither substitution fires; the proof only normalizes the
two embeddings of that preserved head. -/
private theorem DiamondSite.tauBranch {C : Carrier}
    {pre dom cod τ src dst : Shape C}
    [Proper dom] [Proper cod] [Proper τ] [Proper src] [Proper dst]
    (σ : Subst C dom (pre ⋈ cod))
    {υ : List C.Arity}
    ...
    Subst.act κσ (Tele.ofList υ)
      (actAt (pre := pre) σ (Proper.underList hτSrc υ)
        (.ap (DiamondSite.embed hτSrc (.tau xτ)) args))
    =
    σ.act ((τ ⋈ dst) ⋈ Tele.ofList υ)
      (κ.act (Tele.ofList υ)
        (.ap (DiamondSite.embed hτSrc (.tau xτ)) args)) := by
  refine (congrArg (Subst.act κσ (Tele.ofList υ))
  ...
  refine (Subst.act_ap_left κσ (Tele.ofList υ)
    ((Proper.inr (pre ⋈ cod)) xτ)
    (fun j => σ.act (((τ ⋈ src) ⋈ Tele.ofList υ) ∷ j.arity) (args j))).trans ?_
  simp only [Proper.extendList_inr_inl, srcTarget.inr_left]
  rw [show
    ...
    Proper.inl (((pre ⋈ dom) ⋈ τ) ⋈ src)
      ((Proper.inl ((pre ⋈ dom) ⋈ τ))
        ((Proper.inr (pre ⋈ dom)) xτ)) by rfl]
  rw [Subst.act_ap_left κ (Tele.ofList υ)
    ((Proper.inr (pre ⋈ dom)) xτ) args]
  simp only [(dstTarget.inr_left (pre ⋈ dom) xτ).symm]
  rw [show
    Proper.inl (((pre ⋈ dom) ⋈ τ) ⋈ dst)
      ((Proper.inr (pre ⋈ dom)) ((Proper.inl τ) xτ))
      =
    Proper.inl ((pre ⋈ dom) ⋈ (τ ⋈ dst))
      ((Proper.inr (pre ⋈ dom)) ((Proper.inl τ) xτ)) by rfl]
  rw [← Proper.extendList_inr_inl (τ ⋈ dst) υ (pre ⋈ dom)
    ((Proper.inl τ) xτ)]
  refine Eq.trans ?_ (Subst.act_ap_right σ ((τ ⋈ dst) ⋈ Tele.ofList υ)
    ((Proper.inl (τ ⋈ dst)) ((Proper.inl τ) xτ))
    (fun j => Subst.act κ (Tele.ofList υ ∷ j.arity) (args j))).symm
  simp only [Proper.extendList_inr_inl, dstTarget.inr_left]
  congr 1
  funext j
  exact hargs j

/- Stable prefix heads are preserved by both substitutions.  As in the `τ`
case, the proof is only reassociation of the preserved head plus the recursive
argument equalities. -/
private theorem DiamondSite.preBranch {C : Carrier}
    {pre dom cod τ src dst : Shape C}
    [Proper dom] [Proper cod] [Proper τ] [Proper src] [Proper dst]
    (σ : Subst C dom (pre ⋈ cod))
    {υ : List C.Arity}
    ...
    Subst.act κσ (Tele.ofList υ)
      (actAt (pre := pre) σ (Proper.underList hτSrc υ)
        (.ap (DiamondSite.embed hτSrc (.pre z)) args))
    =
    σ.act ((τ ⋈ dst) ⋈ Tele.ofList υ)
      (κ.act (Tele.ofList υ)
        (.ap (DiamondSite.embed hτSrc (.pre z)) args)) := by
  refine (congrArg (Subst.act κσ (Tele.ofList υ))
  ...
  refine (Subst.act_ap_left κσ (Tele.ofList υ)
    ((Proper.inl (pre ⋈ cod)) ((Proper.inl pre) z))
    (fun j => σ.act (((τ ⋈ src) ⋈ Tele.ofList υ) ∷ j.arity) (args j))).trans ?_
  simp only [Proper.extendList_inl, srcTarget.inl]
  rw [show
    Proper.inl (pre ⋈ dom ⋈ (τ ⋈ src))
      ((Proper.inl ((pre ⋈ dom) ⋈ τ))
    ...
    Proper.inl (((pre ⋈ dom) ⋈ τ) ⋈ src)
      ((Proper.inl ((pre ⋈ dom) ⋈ τ))
        ((Proper.inl (pre ⋈ dom)) ((Proper.inl pre) z))) by rfl]
  rw [Subst.act_ap_left κ (Tele.ofList υ)
    ((Proper.inl (pre ⋈ dom)) ((Proper.inl pre) z)) args]
  simp only [(dstTarget.inl (pre ⋈ dom) ((Proper.inl pre) z)).symm]
  change _ =
    σ.act ((τ ⋈ dst) ⋈ Tele.ofList υ)
      (.ap
        ((Proper.inl ((pre ⋈ dom) ⋈ (τ ⋈ dst)))
          ((Proper.inl (pre ⋈ dom)) ((Proper.inl pre) z)))
        (fun i => Subst.act κ (Tele.ofList υ ∷ i.arity) (args i)))
  rw [← Proper.extendList_inl (τ ⋈ dst) υ (pre ⋈ dom)
    ((Proper.inl pre) z)]
  refine Eq.trans ?_ (Subst.act_ap_left σ ((τ ⋈ dst) ⋈ Tele.ofList υ) z
    (fun j => Subst.act κ (Tele.ofList υ ∷ j.arity) (args j))).symm
  simp only [Proper.extendList_inl, dstTarget.inl]
  congr 1
  funext j
  exact hargs j
...
private theorem diamondAt
  ...
  | .ap (α := β) head args =>
      ...
      | under xυ =>
          ...
          exact DiamondSite.underBranch σ hτSrc hτDst srcTarget κ κσ xυ args hargs
      | src xsrc =>
          -- Source head for `κ`.  The right-hand route fires `κ` at `xsrc`.
          -- The left-hand route first acts on the head and then fires the
          -- postcomposed substitution.  Both reduce to the smaller diamond on the
          ...
          let κβ : Subst C ⌊β⌋ (pre ⋈ dom ⋈ τ ⋈ dst ⋈ Tele.ofList υ) :=
            Subst.actPointwise (pre := (pre ⋈ dom) ⋈ τ) κ υ args
          ...
          have hSubst :
            ...
            | there q => nomatch q
          rw [hSubst]
          simpa only [κβ, Subst.actPointwise, Subst.fromArgs] using hrec
      | tau xτ =>
          ...
          exact DiamondSite.tauBranch σ hτSrc hτDst srcTarget dstTarget
            κ κσ xτ args hargs
      | dom z =>
      ...
      | pre z =>
          ...
          exact DiamondSite.preBranch σ hτSrc hτDst srcTarget dstTarget
            κ κσ z args hargs
termination_by
...
private theorem liftAt
  ...
  let lam :  Subst C ⌊β⌋ (pre ⋈ (τ ⋈ src ⋈ Tele.ofList υ)) :=
    Subst.fromArgs β ((τ ⋈ src) ⋈ Tele.ofList υ) (pre := pre) η
  let lam' : Subst C ⌊β⌋ (pre ⋈ (τ ⋈ dst ⋈ Tele.ofList υ)) :=
    Subst.fromArgs β ((τ ⋈ dst) ⋈ Tele.ofList υ)
      (pre := pre) (fun j => κ.act (Tele.ofList υ ∷ j.arity) (η j))
  ...
  | .ap (α := γ) head args =>
      ...
      | beta j =>
          ...
          let θSubst : Subst C ⌊j.arity⌋ (pre ⋈ τ ⋈ src ⋈ Tele.ofList υ ⋈ Tele.ofList χ) :=
            Subst.fromArgs j.arity (Tele.ofList χ)
...
theorem Subst.act_comp
  ...
  act (kcomp σ θ) τ e = θ.act τ (σ.act τ e)
  := by
  let σθ : Subst C dom (pre ⋈ cod) := kcomp σ θ
  change act σθ τ e = θ.act τ (σ.act τ e)
  match e with
  | .ap (α := β) head args =>
    ...
    | right x =>
      ...
      refine (Subst.act_ap_right σθ τ x args).trans ?_
      change
        .ap ((Proper.inr (pre ⋈ cod)) x)
          (fun j => act σθ (τ ∷ j.arity) (args j))
        =
        θ.act τ
          (σ.act τ
    ...
    | middle y =>
      ...
      refine (Subst.act_ap_middle σθ τ y args).trans ?_
      have ih_args :=
      ...
      let κβ : Subst C ⌊β⌋ (pre ⋈ mid ⋈ τ) := Subst.fromArgs β τ (fun i => σ.act (τ ∷ i.arity) (args i))
      have h := diamondAt
        (τ := Shape.nil)
        θ
        (υ := [])
        ...
        κβ (e := σ y)
      unfold Subst.comp at h
      unfold κβ at h
      unfold Subst.fromArgs at h
      dsimp only at h
      change
...
:= by
    ...
    | left z =>
      ...
      refine (Subst.act_ap_left σθ τ z args).trans ?_
      change
        .ap ((Proper.inl (pre ⋈ cod)) ((Proper.inl pre) z))
          (fun i => Subst.act σθ (τ ∷ i.arity) (args i))
        =
        θ.act τ
          (σ.act τ
            (.ap
