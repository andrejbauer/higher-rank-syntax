...
private theorem subst_extendList_weakenList {C : Carrier} {Γ Δ : Shape C}
  ...
  | β :: rest, α, q => by
    show ⟦ Renaming.weaken (Δ ⋈* rest) β ⇑ʳ α ⟧ʳ
            ((σ ⇑ˢ* rest) ((Γ ↪ʳ* rest) q))
        = ⟦ (Renaming.weaken (Δ ⋈* rest) β ∘ʳʳ (Δ ↪ʳ* rest)) ⇑ʳ α ⟧ʳ (σ q)
...
`⟦ ρ ⇑ʳ* τ ⟧ʳ` equals walking `lift.aux (ρ ∘ʳˢ σ) τ` directly.  The renaming
...
private theorem inst_aux_factor_ren {C : Carrier} :
    ∀ {Δ Ξ : Shape C} (α : C.Arity) (ρ : Δ →ʳ Ξ) (ι : Inst α Ξ)
      ...
      inst.aux ρ ι τ e
        = inst.aux 𝟙ʳ ι τ (⟦ (ρ ⇑ʳ α) ⇑ʳ* τ ⟧ʳ e)
  | Δ, Ξ, α, ρ, ι, τ, .apply (α := α_h) p args => by
    have ih_arg : ∀ (j : C.Binder α_h),
        inst.aux ρ ι (j.arity :: τ) (args j)
          = inst.aux 𝟙ʳ ι (j.arity :: τ)
              (⟦ (ρ ⇑ʳ α) ⇑ʳ* (j.arity :: τ) ⟧ʳ (args j)) := by
      intro j
...
private theorem inst_aux_rename_id_inner {C : Carrier} {Δ Δ' : Shape C} (α : C.Arity)
    (ih_α : ∀ (j : C.Binder α) {Δ_ Δ'_ : Shape C} (ρ_ : Δ_ →ʳ Δ'_)
            ...
            ⟦ ρ_ ⇑ʳ* τ_ ⟧ʳ (inst.aux 𝟙ʳ ι_ τ_ e_)
              = inst.aux 𝟙ʳ (fun k => ⟦ ρ_ ⇑ʳ k.arity ⟧ʳ (ι_ k)) τ_
                  (⟦ (ρ_ ⇑ʳ j.arity) ⇑ʳ* τ_ ⟧ʳ e_))
    (ρ : Δ →ʳ Δ') (ι : Inst α Δ) :
    ∀ (τ : List C.Arity) (e : Expr ((Δ ⋈ α) ⋈* τ)),
      ⟦ ρ ⇑ʳ* τ ⟧ʳ (inst.aux 𝟙ʳ ι τ e)
        = inst.aux 𝟙ʳ (fun k => ⟦ ρ ⇑ʳ k.arity ⟧ʳ (ι k)) τ
            (⟦ (ρ ⇑ʳ α) ⇑ʳ* τ ⟧ʳ e)
  | τ, .apply (α := α_h) p args => by
    have ih_arg : ∀ (k : C.Binder α_h),
        ⟦ ρ ⇑ʳ* (k.arity :: τ) ⟧ʳ
            (inst.aux 𝟙ʳ ι (k.arity :: τ) (args k))
          = inst.aux 𝟙ʳ (fun j => ⟦ ρ ⇑ʳ j.arity ⟧ʳ (ι j))
              (k.arity :: τ)
              (⟦ (ρ ⇑ʳ α) ⇑ʳ* (k.arity :: τ) ⟧ʳ (args k)) :=
    ...
    | base q =>
      ...
      | here j =>
        ...
        have ih_j := ih_α j (ρ ⇑ʳ* τ)
              (fun k => inst.aux 𝟙ʳ ι (k.arity :: τ) (args k)) []
              (⟦ (Δ ↪ʳ* τ) ⇑ʳ j.arity ⟧ʳ (ι j))
        simp only [Renaming.extendList_nil] at ih_j
        refine ih_j.trans ?_
        -- Goal: inst.aux 𝟙ʳ Λ_LHS [] (⟦ρ ⇑ʳ* τ ⇑ʳ j.arity⟧ʳ (⟦(Δ↪ʳτ) ⇑ʳ j.arity⟧ʳ ι j))
        --     = inst.aux 𝟙ʳ Λ_RHS [] (⟦(Δ' ↪ʳ* τ) ⇑ʳ j.arity⟧ʳ (⟦ρ ⇑ʳ j.arity⟧ʳ ι j))
        congr 1
        ...
        · -- value position equality via renaming naturality
          ...
          show Renaming.actExpr (((ρ ⇑ʳ* τ) ⇑ʳ j.arity) ∘ʳʳ
                                  ((Δ ↪ʳ* τ) ⇑ʳ j.arity)) (ι j) = _
          rw [← Renaming.extend_comp (Δ ↪ʳ* τ) (ρ ⇑ʳ* τ) j.arity,
...
/-- **Inst-Ren naturality** (at the identity-renaming form): pushing a renaming `ρ ⇑ʳ* τ`
past `inst.aux 𝟙ʳ ι τ` factors into a renamed instantiation on the result plus a renaming
...
/-- **SubRen fusion** (Allais et al. F3): walking `⟦ρ ⇑ʳ* τ⟧ʳ` after
`lift.aux σ τ` equals walking `lift.aux (σ ∘ˢʳ ρ) τ` directly.  The renaming
...
private theorem ren_after_subst {C : Carrier} :
    ∀ {Γ Δ Δ' : Shape C} (σ : Subst Γ Δ) (ρ : Δ →ʳ Δ')
      (τ : List C.Arity) (e : Expr (Γ ⋈* τ)),
      ⟦ ρ ⇑ʳ* τ ⟧ʳ (lift.aux σ τ e)
        = lift.aux (σ ∘ˢʳ ρ) τ e
  | Γ, Δ, Δ', σ, ρ, τ, .apply (α := α_h) p args => by
    have ih_arg : ∀ (k : C.Binder α_h),
        ⟦ ρ ⇑ʳ* (k.arity :: τ) ⟧ʳ (lift.aux σ (k.arity :: τ) (args k))
          = lift.aux (σ ∘ˢʳ ρ) (k.arity :: τ) (args k) :=
    ...
    | ext i =>
      ...
      exact ih_arg k
    | base q =>
      -- LHS: ⟦ρ ⇑ʳ* τ⟧ʳ (inst.aux α_h (Δ↪ʳτ) Λ_LHS [] (σ q)).
      ...
      have hI := inst_aux_rename_id α_h (ρ ⇑ʳ* τ)
        (fun j => lift.aux σ (j.arity :: τ) (args j)) []
        (⟦ (Δ ↪ʳ* τ) ⇑ʳ α_h ⟧ʳ (σ q))
      simp only [Renaming.extendList_nil] at hI
      refine hI.trans ?_
      -- RHS: lift.aux (σ ∘ˢʳ ρ) τ (apply ((Γ↪ʳτ) q) args).
      rw [lift_aux_base_eq, inst_aux_factor_ren α_h (Δ' ↪ʳ* τ)]
      simp only [Renaming.extendList_nil]
      congr 1
      · funext k
      ...
      · -- Value equality via renaming naturality.
        rw [← Renaming.actExpr.map_comp]
        show Renaming.actExpr (((ρ ⇑ʳ* τ) ⇑ʳ α_h) ∘ʳʳ
                                ((Δ ↪ʳ* τ) ⇑ʳ α_h)) (σ q) = _
        rw [← Renaming.extend_comp (Δ ↪ʳ* τ) (ρ ⇑ʳ* τ) α_h,
            Renaming.weakenList_naturality,
...
private theorem inst_aux_η_inv_of {C : Carrier} (Δ : Shape C) (α : C.Arity)
    (hη : ∀ (j : C.Binder α) {Δ' Ξ' : Shape C}
      ...
      inst.aux ρ ι [] (Expr.η v)
        = Expr.apply (ρ v) ι) :
    ∀ (τ : List C.Arity) (e : Expr ((Δ ⋈ α) ⋈* τ)),
      inst.aux (Δ ↪ʳ* [α]) (η_fillers Δ α) τ e = e
  | τ, .apply (α := α_h) p args => by
    have ih_arg : ∀ (j : C.Binder α_h),
        inst.aux (Δ ↪ʳ* [α]) (η_fillers Δ α)
    ...
    | base q =>
      ...
      | here j =>
        ...
        change inst.aux (Δ ⋈ α ↪ʳ* τ)
            (fun k => inst.aux (Δ ↪ʳ* [α]) (η_fillers Δ α)
              (k.arity :: τ) (args k)) []
            (Expr.η (.here j))
...
private theorem inst_aux_η {C : Carrier} {Δ Ξ : Shape C}
    ...
    inst.aux ρ ι [] (Expr.η v) = Expr.apply (ρ v) ι := by
  unfold Expr.η
  change inst.aux ρ ι []
  ...
  funext i
  unfold Expr.η
  change inst.aux ρ ι [i.arity]
      (Expr.apply ((Δ ⋈ α ↪ʳ* [i.arity]) (.here i))
        (fun k => Expr.η (.here k)))
      = ι i
  rw [inst_aux_base_here_eq]
  change inst.aux (Ξ ↪ʳ* [i.arity])
      (fun k : C.Binder i.arity => inst.aux ρ ι (k.arity :: [i.arity])
        (Expr.η (.here k))) [] (ι i)
    = ι i
  have hargs : (fun k : C.Binder i.arity => inst.aux ρ ι (k.arity :: [i.arity])
        (Expr.η (.here k)))
...
private theorem lift_toSubst.aux {C : Carrier} {Γ Δ : Shape C} (ρ : Γ →ʳ Δ) :
    ∀ (τ : List C.Arity) (e : Expr (Γ ⋈* τ)),
      lift.aux ρ.toSubst τ e = ⟦ ρ ⇑ʳ* τ ⟧ʳ e
  | τ, .apply (α := α_h) p args => by
    have ih_arg : ∀ (j : C.Binder α_h),
        lift.aux ρ.toSubst (j.arity :: τ) (args j)
          = ⟦ ρ ⇑ʳ* (j.arity :: τ) ⟧ʳ (args j) := by
      intro j
...
private theorem lift_aux_via_extendList {C : Carrier} :
  ...
  | Γ, Δ, σ, τ, .apply (α := α_h) p args => by
    ...
    | ext i =>
      rename_i ta b tb
      rw [lift_aux_ext_eq]
      refine Eq.trans ?_ (lift_aux_nil_apply (σ ⇑ˢ* (ta ++ b :: tb))
    ...
    | base q =>
      rw [lift_aux_base_eq, inst_aux_factor_ren α_h (Δ ↪ʳ* τ)]
      simp only [Renaming.extendList_nil]
      refine Eq.trans ?_ (lift_aux_nil_apply (σ ⇑ˢ* τ)
        ((Γ ↪ʳ* τ) q) args).symm
...
private theorem lift_inst_commute {C : Carrier} :
    ∀ {Δ Ε : Shape C} (θ : Subst Δ Ε) (α : C.Arity)
      ...
      (e : Expr ((Δ ⋈ α) ⋈* ρ)),
      lift.aux θ (ρ ++ τ)
        (Shape.extList_append Δ ρ τ ▸ inst.aux (Δ ↪ʳ* τ) ι ρ e)
        =
...
private theorem comp_lift.aux {C : Carrier} {Γ Δ Ε : Shape C}
    ...
    ∀ (τ : List C.Arity) (e : Expr (Γ ⋈* τ)),
      lift.aux (σ ∘ˢˢ θ) τ e = lift.aux θ τ (lift.aux σ τ e)
  | τ, .apply (α := α_h) p args => by
    have ih_arg : ∀ (j : C.Binder α_h),
        lift.aux (σ ∘ˢˢ θ) (j.arity :: τ) (args j)
