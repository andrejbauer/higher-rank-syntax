...
theorem Wf_e.eta : ∀ {Δ α : C.Arity} (Ξ : Ambient Δ) (x : Δ ∋ α),
    ...
    Wf_e (Ξ.extend (Ξ.binding x)) (Expr.η x)
  | Δ, α, Ξ, x, hb, hx => by
      rw [Expr.η.eq_1]
      ...
      case fill =>
        ...
        exact congrArg (fun T => Wf_s (Ξ.extend (Ξ.binding x)) T (Subst.instId Δ α))
...
theorem Wf_s.eta : ∀ {Δ Ω : C.Arity} (Ξ : Ambient Δ) (Θ : dTel Δ Ω),
    Wf_t Ξ Θ → Wf_s (Ξ.extend Θ) (dTel.rename (Renaming.inl Δ Ω) Θ) (Subst.instId Δ Ω)
  | Δ, Ω, Ξ, Θ, hΘ => by
      ...
      case equation =>
        ...
        have hbd := (h ▸ Wf_t.declaration hΘ z :
          Wf_bd (Ξ.extend Θ) (Θ.binding z) (Bd.eq l r))
        refine Eq.mp ?_ (Eq_e.hyp (Ξ := (Ξ.extend Θ).extend (Θ.binding z))
          (C.inl (C.inr z)) (⟦ Renaming.inl (Δ ⋈ Ω) Λ ⇑ʳ Λ ⟧ʳ l)
          (⟦ Renaming.inl (Δ ⋈ Ω) Λ ⇑ʳ Λ ⟧ʳ r) (Subst.instId (Δ ⋈ Ω) Λ)
          ?decl ?hl ?hr ?fill)
        case decl =>
          refine Eq.trans (dTel.declaration_concatenate_inl (Ξ.extend Θ)
        ...
        case hl =>
          refine Eq.mp ?_ (Wf_e.weaken
            ((Ambient.Renaming.weaken (Ξ.extend Θ) (Θ.binding z)).extend (Θ.binding z))
            hbd.1)
          exact congrArg (fun T => Wf_e (((Ξ.extend Θ).extend (Θ.binding z)).extend T)
              (⟦ Renaming.inl (Δ ⋈ Ω) Λ ⇑ʳ Λ ⟧ʳ l))
            (((dTel.binding_concatenate_inl (Ξ.extend Θ) (Θ.binding z) (C.inr z)).trans
          ...
          exact congrArg (fun T => Wf_e (((Ξ.extend Θ).extend (Θ.binding z)).extend T)
            ...
            (((dTel.binding_concatenate_inl (Ξ.extend Θ) (Θ.binding z) (C.inr z)).trans
              (congrArg (dTel.rename (Renaming.inl (Δ ⋈ Ω) Λ))
                (dTel.binding_concatenate_inr Ξ Θ z))).symm)
        case fill =>
          refine Eq.mp ?_ (Wf_s.eta (Ξ.extend Θ) (Θ.binding z) (Wf_t.binding hΘ z))
          exact congrArg (fun T => Wf_s ((Ξ.extend Θ).extend (Θ.binding z)) T
              (Subst.instId (Δ ⋈ Ω) Λ))
            (((dTel.binding_concatenate_inl (Ξ.extend Θ) (Θ.binding z) (C.inr z)).trans
              (congrArg (dTel.rename (Renaming.inl (Δ ⋈ Ω) Λ))
                (dTel.binding_concatenate_inr Ξ Θ z))).symm)
        ...
        refine Eq.trans (congrArg₂ (Eq_e ((Ξ.extend Θ).extend (Θ.binding z)))
          (hcancel l) (hcancel r)) ?_
        exact congrArg (fun T => Eq_e ((Ξ.extend Θ).extend T) l r)
          (dTel.instantiate_binding_instId Θ z).symm
      case filler =>
        intro Λ z hne
        have hx : ¬ ((Ξ.extend Θ).declaration (C.inr z)).isEq := by
          refine fun hEq => hne ?_
          exact (dTel.act_declaration_instId Θ z).symm ▸
            ((dTel.declaration_concatenate_inr Ξ Θ z) ▸ hEq)
        have hb : Wf_t (Ξ.extend Θ) ((Ξ.extend Θ).binding (C.inr z)) := by
          refine Eq.mp ?_ (Wf_t.binding hΘ z)
          exact congrArg (fun T => Wf_t (Ξ.extend Θ) T)
            (dTel.binding_concatenate_inr Ξ Θ z).symm
        refine Eq.mp ?_ (Wf_e.eta (Ξ.extend Θ) (C.inr z) hb hx)
        exact congrArg (fun T => Wf_e ((Ξ.extend Θ).extend T) (Expr.η (C.inr z)))
          ((dTel.binding_concatenate_inr Ξ Θ z).trans
            (dTel.instantiate_binding_instId Θ z).symm)
      case declared =>
        intro Λ z hne
        have hB : ((Ξ.extend Θ).extend (Θ.binding z)).boundaryOf (Expr.η (C.inr z))
            = Θ.declaration z := by
          refine Eq.trans (congrArg (fun (T : dTel (Δ ⋈ Ω) Λ) =>
            ((Ξ.extend Θ).extend T).boundaryOf (Expr.η (C.inr z)))
            (dTel.binding_concatenate_inr Ξ Θ z).symm) ?_
          exact (dTel.boundaryOf_eta (Ξ.extend Θ) (C.inr z)).trans
        ...
        refine Eq.trans (congrArg₂ (fun (b c : Bd ((Δ ⋈ Ω) ⋈ Λ)) =>
            Eq_bd ((Ξ.extend Θ).extend (Θ.binding z)) b c) hB.symm
          (dTel.act_declaration_instId Θ z).symm) ?_
        exact congrArg (fun (T : dTel (Δ ⋈ Ω) Λ) =>
            Eq_bd ((Ξ.extend Θ).extend T)
              (((Ξ.extend Θ).extend T).boundaryOf (Expr.η (C.inr z)))
