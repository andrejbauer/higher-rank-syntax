...
`ArityMod T` is the slice of `T`-modules over this constant functor.  Thus an
object is a syntax module together with a substitution-invariant shape map.
For decorated telescopes the module element is a telescope, and its shape is
its undecorated raw arity.  Replacing `A` by `B` in the tail `x : A` changes
the classifier but leaves the one-slot shape unchanged.

This file only remembers module action and shape.  The dependent sequencing
of shaped modules is the tensor in `TelescopeTensor`; the particular shaped
module of decorated telescopes and its monoid operations are constructed
later.  None of these structures asserts that a classifier is well formed.
-/
